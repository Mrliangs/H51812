...
(function (global, document, undefined) {
	var rblock = /\{([^\}]*)\}/g,
		...
		noop: function noop() {},
		//Object
		mix: function mix(target, source, cover) {
		...
		//Number
		getRndNum: function getRndNum(max, min) {
		...
		//DOM
		loadScriptCache: {},
		loadScript: function loadScript(url, callback, args) {
			...
			if (!cache) {
				...
				script.onload = script.onreadystatechange = script.onerror = function () {
					if (/undefined|loaded|complete/.test(this.readyState)) {
						script = script.onreadystatechange = script.onload = script.onerror = null;
						cache.loaded = true;
		...
		requestAnimationFrame: function () {
			var handler = global.requestAnimationFrame || global.webkitRequestAnimationFrame || global.mozRequestAnimationFrame || global.msRequestAnimationFrame || global.oRequestAnimationFrame || function (callback) {
		...
		animate: function () {
			var easeOut = function easeOut(pos) {
			...
			return function (elem, name, to, duration, callback, easing) {
				var from = parseFloat(getCurrCSS(elem, name)) || 0,
				    style = elem.style,
				    tMark = new Date(),
				    size = 0;
		...
		setCookie: function setCookie(name, value, expir) {
...
(function (global, document, $, undefined) {
	...
	ds.mask = $.mask = function (opacity, background) {
		var ops = (typeof opacity === 'undefined' ? 'undefined' : _typeof(opacity)) === 'object' ? opacity : {
			opacity: opacity,
...
})(undefined, undefined.document, jQuery);
...
(function (global, document, $, undefined) {
	...
	var _guid = 0,
	    _noop = function _noop() {},
	    _tmpl = '<div class="ds_dialog_outer"><table class="ds_dialog_border"><tr><td class="ds_dialog_tl"></td><td class="ds_dialog_tc"></td><td class="ds_dialog_tr"></td></tr><tr><td class="ds_dialog_ml"></td><td class="ds_dialog_mc"><div class="ds_dialog_inner"><table class="ds_dialog_panel"><tr><td colspan="2" class="ds_dialog_header"><div class="ds_dialog_title"><h3></h3></div><div class="ds_dialog_close"><a href="javascript:;">\xD7</a></div></td></tr><tr><td class="ds_dialog_icon" style="display:none"><div class="ds_dialog_icon_bg"></div></td><td class="ds_dialog_main"><div id="{id}_content" class="ds_dialog_content"></div></td></tr><tr><td colspan="2" class="ds_dialog_footer"><div class="ds_dialog_buttons"></div></td></tr></table></div></td><td class="ds_dialog_mr"></td></tr><tr><td class="ds_dialog_bl"></td><td class="ds_dialog_bc"></td><td class="ds_dialog_br"></td></tr></table></div>',
	    _ops = {
		id: null,
		title: null,
		content: '',
	...
	$.extend(Dialog, {
		...
		removeDialog: function removeDialog(dialog) {
	...
	Dialog.prototype = {
		...
		_getElem: function _getElem(ops) {
		...
		_getPositionLimit: function _getPositionLimit() {
			var ops = this.ops,
			    shell = this.shell,
			    offset = shell.offset(),
		...
		_position: function _position(left, top) {
			var ops = this.ops,
		...
		padding: function padding(_padding) {
			this.contentShell.css('padding', _padding);
			return this;
		},
		follow: function () {
			var _offsetMaps = {
				...
				right: function right(shell, target) {
					return target.outerWidth() - shell.outerWidth();
				},
				top: function top(shell, target) {
					return -shell.outerHeight();
				},
				bottom: function bottom(shell, target) {
					return target.outerHeight();
				}
			},
			    _getOffset = function _getOffset(shell, target, _offset) {
		...
		title: function title(_title) {
			if (!_title) {
				...
				this.titleShell.html('<h3>' + _title + '</h3>');
				this.titleShell.show();
			}
			return this;
		},
		content: function content(_content) {
			this.contentShell.html(_content);
			return this.position(this.ops.left, this.ops.top);
		},
		button: function button(buttons) {
			var self = this,
			    ops = this.ops,
		...
		lock: function lock(opacity, lockColor) {
			var ops = this.ops,
			    mask = Dialog.mask;
			if (!mask) {
	...
	$.extend(Dialog.prototype, function () {
		...
		return {
			...
			releaseFixed: function releaseFixed() {
				if (this._initFixed) {
	...
	$.extend(Dialog.prototype, function () {
		var html = document.documentElement,
		    ...
		    isNotDragArea = function isNotDragArea(dialog, target) {
		...
		return {
			...
			releaseDrag: function releaseDrag() {
				if (this._dragHandler) {
	...
	$.extend(ds.dialog, {
		items: Dialog.items,
		alert: function alert(content, onhide, icon) {
		...
		confirm: function confirm(content, onyes, onno, onhide) {
			return new Dialog({
		...
		prompt: function prompt(content, _onyes, defaultValue) {
			return new Dialog({
				id: 'ds_dialog_prompt',
				fixed: true,
				left: '50%',
				...
				onyes: function onyes() {
					var input = this._input[0];
					return typeof _onyes === 'function' ? _onyes.call(this, input.value, input) : void 0;
				},
				onno: true
			});
		},
		tips: function tips(content, timeout, follow, lock) {
